...
class NmtModel(object):
    ...
    """
        ...
        embedding_source_words = embedding_source(source_words)
        embedding_source_words = Dropout(rate=self.embedding_dropout_rate)(embedding_source_words)
        embedding_target_words = embedding_target(target_words)
        embedding_target_words = Dropout(rate=self.embedding_dropout_rate)(embedding_target_words)

        encoder = LSTM(units=self.hidden_size, return_sequences=True, recurrent_dropout=self.hidden_dropout_rate,
                       return_state=True)
        encoder_outputs, encoder_state_h, encoder_state_c = encoder(embedding_source_words)
    ...
    """
        ...
        decoder_lstm = LSTM(self.hidden_size, recurrent_dropout=self.hidden_dropout_rate, return_sequences=True,
                            return_state=True)
        decoder_outputs_train, _, _ = decoder_lstm(embedding_target_words, initial_state=encoder_states)
    ...
    """
        ...
        decoder_outputs_test, decoder_state_output_h, decoder_state_output_c = decoder_lstm(embedding_target_words,
    ...
    def eval(self, dataset):
        ...
        for _ in range(self.max_target_step):
            step_decoder_outputs, state_h, state_c = self.decoder_model.predict(
                [step_target_words, state_h, state_c, encoder_outputs], batch_size=self.batch_size)
...
class AttentionLayer(Layer):
    ...
    Start
    """
        decoder_outputs1 = K.permute_dimensions(decoder_outputs, (0, 2, 1))
        luong = K.batch_dot(encoder_outputs, decoder_outputs1)
        luong = K.softmax(luong, axis = 1)
        luong = K.expand_dims(luong, axis=-1)
        encoder_outputs = K.expand_dims(encoder_outputs, axis=2)
        # encoder_outputs = K.permute_dimensions(encoder_outputs, (0, 1, 3, 2))
        encoder_vector = encoder_outputs * luong
...
def load_dataset(source_path, target_path, max_num_examples=30000):
    ...
    target_words = [[target_lang_dict.word2ids.get(tok, target_lang_dict.UNK) for tok in sent[:-1]] for sent in
                    target_sents]
...
if __name__ == '__main__':
    max_example = 30000
    use_attention = True
    train_data, dev_data, test_data, source_dict, target_dict = load_dataset("data.30.vi", "data.30.en",
